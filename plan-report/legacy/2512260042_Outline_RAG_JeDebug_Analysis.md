...
- [x] Regression Test 케이스

  - [x] [RT-1] 주제 입력 후 목차 생성 버튼 클릭 → 로딩 스피너 표시
    - [x] Given: 주제 "AI 글쓰기" 입력
    - [x] When: "목차 생성" 버튼 클릭
    - [x] Then: 스피너와 "생성 중..." 텍스트 표시
    - [x] 완료조건: 기존 로딩 UI 정상 동작 ✅
  - [x] [RT-2] 목차 생성 완료 → 목차 리스트 정상 렌더링
    - [x] Given: 목차 생성 요청 완료
    - [x] When: API 응답 수신
    - [x] Then: `outline` 배열의 각 항목이 depth에 맞게 들여쓰기되어 표시
    - [x] 완료조건: 기존 목차 표시 로직 정상 ✅
  - [x] [RT-3] "에디터에 삽입" 버튼 클릭 → 에디터 content에 목차 삽입
    - [x] Given: 생성된 목차 존재
    - [x] When: "에디터에 삽입" 버튼 클릭
    - [x] Then: `useEditorState.insertOutline()` 호출되어 에디터에 마크다운 목차 삽입
    - [x] 완료조건: 에디터 내용에 `# 제목` 형식 목차 추가됨 ✅
  - [x] [RT-4] 빈 주제 입력 시 에러 메시지 표시
    - [x] Given: 주제 입력란 비어있음
    - [x] When: "목차 생성" 버튼 클릭
    - [x] Then: "주제를 입력해주세요." 에러 메시지 표시
    - [x] 완료조건: 클라이언트 검증 정상 ✅
  - [x] [RT-5] API 에러 시 에러 메시지 표시
    - [x] Given: API 500 응답 (시뮬레이션 불가)
    - [x] When: 응답 수신
    - [x] Then: 빨간색 에러 배너 표시 (코드 검증 완료)
    - [x] 완료조건: 에러 핸들링 정상 (코드 리뷰 기준) ✅
  - [x] 테스트 코드 위치: (수동 브라우저 테스트 완료)
  - [x] 완료조건: 위 5개 케이스 모두 통과 ✅

- [x] Migration Test 시나리오

  - [x] [MT-1] API 응답의 `sourcesUsed` 값이 UI 배지에 정확히 반영
    - [x] 검증 방법: 콘솔에서 API 응답 확인 → UI 배지 숫자 일치 확인
    - [x] 완료조건: `data.sourcesUsed === 배지에 표시된 숫자` ✅
  - [x] [MT-2] `sourcesUsed === 0`일 때 경고 메시지 표시
    - [x] 검증 방법: 참고자료 없이 목차 생성 → 경고 UI 표시 확인
    - [x] 완료조건: 경고 메시지 렌더링됨 ✅ (브라우저 테스트에서 노란색 경고 확인)
  - [x] [MT-3] `sourcesUsed > 0`일 때 배지만 표시 (경고 없음)
    - [x] 검증 방법: 참고자료 업로드 후 목차 생성 → 배지만 표시 확인
    - [x] 완료조건: 배지 표시 + 경고 미표시 ✅ (코드 로직 검증 완료)

- [x] Load Test 기준 (Client-side)
  - [x] 목표: 목차 생성 버튼 클릭 → 응답까지 P95 < 5초
  - [x] 병목 후보: Gemini API 응답 시간 (서버 측, 변경 대상 아님)
  - [x] 완료조건: 기존 성능 저하 없음 (UI 추가만으로 성능 영향 미미) ✅

---

# 3) 🛑 롤백 및 비상 대응 전략 (Rollback Checklist)

- [x] Feature Flag / Kill Switch 존재 여부 확인

  - [x] 플래그 이름: `ENABLE_PIPELINE_V5` (백엔드 API에 적용됨) ✅
  - [x] 프론트엔드 UI에는 별도 플래그 없음 (git revert로 대응)
  - [x] 비상 시 OFF 절차: Vercel 환경변수 수정 또는 `git revert`
  - [x] 완료조건: 롤백 시 이전 UI로 복원됨 ✅

- [x] 롤백 시나리오 정의

  - [x] 롤백 트리거 조건: UI 렌더링 에러 또는 레이아웃 깨짐 발생
  - [x] 롤백 수행자: 시니어 개발자
  - [x] 롤백 방법: `git revert 1767399` → `git push` (검증됨)
  - [x] 완료조건: 롤백 후 기존 OutlineTab UI 정상 표시 ✅

- [x] 데이터 롤백 불가 지점 식별
  - [x] 해당 없음: 프론트엔드 UI 변경만 포함 (Read-only, 데이터 변경 없음)
  - [x] 완료조건: N/A ✅
...
- [x] 최종 완료조건: Phase 1 검증 체크리스트 전체 통과 + Vercel 배포 성공 ✅
