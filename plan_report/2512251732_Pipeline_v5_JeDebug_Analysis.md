...
- [x] **롤백 트리거 조건 정의**
  - [x] 에러율 > 5% (기존 대비 2배 이상)
  - [x] 평균 응답 시간 > 10초 (기존 대비 2배 이상)
  - [x] 패치 적용 성공률 < 80%
- [x] **롤백 수행자/승인자 정의**
  - [x] 수행자: 시니어 개발자
  - [x] 승인자: 디렉터
- [x] **롤백 절차**
  - [x] Step 1: Feature Flag OFF
  - [x] Step 2: Vercel 이전 배포로 Rollback
  - [x] Step 3: 캐시 무효화 (Redis/IndexedDB) (`clearCriteriaPackCache()` 구현 완료)
  - [x] Step 4: 원인 분석 후 핫픽스
- [x] 완료조건: 롤백 후 v4 정상 동작

### 데이터 롤백 불가 지점

- [x] **롤백 불가 트랜잭션 목록**
  - [x] CriteriaPack Pin 상태: 롤백 시 초기화 필요
  - [x] 패치 적용 이력: 삭제 가능 (DROP 허용)
  - [x] chunk_type 분류: 롤백 시 'general'로 원복 스크립트 필요
- [x] **완화책**
  - [x] 모든 신규 테이블에 soft delete 적용
  - [x] 마이그레이션 롤백 스크립트 사전 작성
  - [x] 데이터 변경 전 백업 스냅샷 생성
- [x] 완료조건: 실패 시에도 데이터 오염 방지 확인
